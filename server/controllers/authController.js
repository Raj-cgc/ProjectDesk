...
export const login = asyncHandler(async (req, res, next) => { 
    ...
    if(!isPasswordMatch){
        return next(new ErrorHandler("Invalid email or password or role", 400));
    }
    generateToken(user, 201, "User logged in successfully", res);
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    try {
        await sendEmail({
            to: user.email,
            subject: "ProjectDesk - 🔐 Password Reset Request",
            message,
        });
        res.status(200).json({
        success: true,
